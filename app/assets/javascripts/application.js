...
$(document).ready(function() {
    ...
    // #FIXME_AB: Make a gridPage object 
    // gridPage = new gridPage();
    // gridPage.adjustPageHeight;
    // gridPage.bindCategoryNavLinks();
    // gridPage.displayFilteredProducts();
    // gridPage.bindUrlHashChange();

    // #FIXME_AB: should be a function 
...
function CategoryProduct() {
    ...
    // #FIXME_AB: better suits as gridPage.displayFilteredProducts
    this.extractDataAndDisplayProducts = function() {
        // #FIXME_AB: Parsing hash would be another method 
    ...
    this.displayContainerGrid = function() {
        var display_container = $('<div>', {
            id: 'display-container',
            class: 'span8'
        });
        $('#main-container').html(display_container);
        display_container.append($('<div>', {
    ...
    this.createLatestProductContainer = function(product, color) {
        return ($('<div>', {
            ...
            'id': "product_" + color['id'],
            'data-color': color.name,
            'data-brand': product.brand.name,
            'data-price': this.getMinPrice(color)
        }));
    }
    ...
    this.displayProductImage = function(product, color, image) {
        img_container.append($('<img>', {
            class: 'latest-color-image',
            'src': image
        })).append($('<a/>', {
                class: 'quick-view visibility btn',
                'data-href': ('/products/' + product.id + '/colors/' + color.id)
            })
            .html('Quick View'));
    }
    ...
    this.getDescription = function(product, latest_product) {
        var latest_products_desc = $('<div>', {
            class: 'latest-products-desc'
        }).append('<div><h5>' + product['title'] + '</h5></div>')
    ...
    this.getFilters = function(filterElements) {
        ...
        $('<div/>', {
            id: 'side-panel'
        }).appendTo('#main-container').append(filterContainer);
    }
